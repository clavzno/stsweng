...
import * as cheerio from 'cheerio'; // for parsing HTML, used in GetCanvasZoomLink
...
export class CanvasService {
    ...
    constructor() {
        this.accessToken = this.GetCanvasAccessToken();
        if (!this.accessToken) {
            throw new Error("Access token must be provided");
        }
        this.baseUrl = "dlsu.instructure.com";
    }
     ...
     */
    async FetchUser() {
        const url = `https://dlsu.instructure.com/api/v1/users/self`;
    ...
    async CreateStudent() {
        ...
        const student = new Student(responseJson, "https://dlsu.instructure.com", this.accessToken);
    ...
    async GetCanvasAccessToken() {
        const response = await fetch('https://dlsu.instructure.com/login/oauth2/token', {
            method: 'POST',
            headers: {
    ...
    async GetCanvasUserInfo() {
        const response = await fetch('dlsu.instructure.com/api/v1/users/self/profile', {
            headers: {
     ...
     */
    async FetchCourses() {
        const url = `https://dlsu.instructure.com/api/v1/courses?${PARAMS_PAGINATION.toString()}`;
    ...
    async GetUserRootFolder() {
        const url = `https://dlsu.instructure.com/api/v1/users/${this.activeStudent.id}/folders/root/`;
    ...
    async GetFilesInFolder(folderId) {
        const url = `https://dlsu.instructure.com/api/v1/folders/${folderId}/files?${PARAMS_PAGINATION.toString()}`;
    ...
    async MakeFilePostRequest(canvasFolderPathId, fileMeta) {
        ...
        const url = `https://dlsu.instructure.com/api/v1/folders/${canvasFolderPathId}/files`;
    ...
    async GetAssignmentsInCourse(courseId) {
        const url = `https://dlsu.instructure.com/api/v1/courses/${courseId}/assignments?${PARAMS_PAGINATION.toString()}`;
    ...
    async AddCommentToAssignment(courseId, assignmentId, comment) {
        const url = `https://dlsu.instructure.com/api/v1/courses/${courseId}/assignments/${assignmentId}/submissions/${this.activeStudent.id}`;
    ...
    async MakeFileUploadRequestToAssignment(courseId, assignmentId) {
        try {
            ...
            const url = `https://dlsu.instructure.com/api/v1/courses/${courseId}/assignments/${assignmentId}/submissions`;
    ...
    /**
     * Gets the course navigation tabs for a given course ID.
     * @param {*} courseId 
     * @returns {Promise<Array>} - Returns an array of course navigation tabs.
     */
    async GetCourseTabs(courseId) {
        const url = `https://dlsu.instructure.com/api/v1/courses/${courseId}/tabs`;
        console.log("Fetching course navigation for course ID: ", courseId, " from: ", url);
        try {
            const res = await fetch(url, {
                headers: {
                    ...HEADER_AUTHORIZATION,
            ...
            if (!res.ok) throw new Error(`Failed to fetch course navigation: ${res.status}`);
            const responseJson = await res.json();
            console.log("GetCourseNavigation > Course navigation fetched successfully: ", responseJson);
            return responseJson;
        } catch (error) {
    ...
    /**
     * Gets the Zoom Meetings link for a given course ID.
     * @param {*} courseId 
     * @returns Zoom Meetings link if it exists, otherwise returns an error message.
     */
    async GetCourseZoomMeetingsUrl(courseId) {
        const url = `https://dlsu.instructure.com/courses/${courseId}/external_tools/6457`;
        try {
            const response = await fetch(url, { credentials: 'include' });
            const html = await response.text();
            const $ = cheerio.load(html);

            // Adjust selector/text as needed for your Canvas instance
            if (!html.includes('Page Not Found')) {
                console.log(url)
                return url;
            } else {
                console.log("No Zoom Meetings tab found for this course.");
                return 'There is no Zoom Meetings tab for this course.';
            }
        } catch (error) {
            return 'Error checking for Zoom Meetings tab.';
        }
    }
}